(function () {
  ...
  const VELOCIDAD_AUTO = 0.5; // Velocidad aumentada solo para el viaje automático
  ...
  let tiempoAnterior = null; // Nueva variable para controlar el tiempo (Delta Time)
  ...
  function mostrarTeclaUI(tecla) {
    ...
    // ESTILOS ACTUALIZADOS PARA LAS TECLAS
    el.style.cssText =
  ...
  function crearJoystickUI() {
    ...
    // ESTILOS ACTUALIZADOS PARA LA BASE DEL JOYSTICK
    base.style.cssText =
      "width:120px;height:120px;background:rgb(229 239 249 / 38%);border-radius:50%;position:relative;border:0.5px solid #6B8BDE;touch-action:none;box-shadow:0 4px 8px rgba(107,139,222,0.3);";

    const knob = document.createElement("div");

    // ESTILOS ACTUALIZADOS PARA LA PALANCA DEL JOYSTICK
    knob.style.cssText =
  ...
  window.camino = function (nombresObjetos, objetivoIrA) {
    ...
    if (!rafId) {
      distanciaActual = 0;
      autoMoviendo = false;
      tiempoAnterior = null; // Reiniciamos el reloj al empezar
    }
